...
and hang on did you just say the sky ("of course, where do you think snow comes from") when you realize that the Elves are 
...
def part_1():
    with open("input_pt_1.txt", 'r') as file:
        total = 0
        for line in file.readlines():
            subbed_string = re.sub(r'[a-z]', '', line).strip()
            if len(subbed_string) == 1:
                new_string = subbed_string * 2
            else:
                new_string = subbed_string[0] + subbed_string[len(subbed_string)-1]
    ...
    with open("input_pt_1.txt", 'r') as file:
        ...
        for line in file.readlines():
            for number in numbers:
                if number[0] in line:
                    line = line.replace(number[0], number[0][0] + number[1] + number[0][len(number)])
