...
def part1():
    with open("input.txt", "r") as f:
        ...
        possible_combinations = {"green": 13, "red": 12, "blue": 14}
        game_id_total = 0

        for game in lines:
            valid = True
            game_id, games = game.strip().split(':')
            all_sets = games.split(';')
            for each_set in all_sets:
                if valid:
                    each_colour = each_set.strip().split(", ")
                    for colour in each_colour:
                        number, label = colour.strip().split(" ")
                        if int(number) > possible_combinations[label]:
                            valid = False
                            break
            if valid:
                _, game_id = game_id.strip().split(" ")
...
def part2():
    with open("input.txt", "r") as f:
        ...
        lowest_combination = {"green": 0, "red": 0, "blue": 0}
        sum_of_powers = 0

        for game in lines:
            lowest_combination = {"green": 0, "red": 0, "blue": 0}
            _, games = game.strip().split(':')
            all_sets = games.split(';')
            for each_set in all_sets:
                each_colour = each_set.strip().split(", ")
                for colour in each_colour:
                    number, label = colour.strip().split(" ")
                    if int(number) > lowest_combination[label]:
                        lowest_combination[label] = int(number)

            cubepower = lowest_combination["green"] * lowest_combination["red"] * lowest_combination["blue"]
            sum_of_powers += cubepower
