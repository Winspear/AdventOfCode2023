...
def part1():
    with open('input.txt', 'r') as myinput:
        ...
        for line in lines:
            _, only_numbers = line.split(':')
            first_string, second_string = only_numbers.split('|') 
            first_numbers = [int(number) for number in first_string.strip().split()]
            second_numbers = [int(number) for number in second_string.strip().split()]
            intersection = set(first_numbers).intersection(second_numbers)
            if intersection:
...
def part2():
    with open('input.txt', 'r') as myinput:
        ...
        for line in lines:
            card_number_string, only_numbers = line.split(':')
            card_number = int(card_number_string.split()[1])
            if not card_numbers.get(str(card_number)):
                card_numbers[str(card_number)] = 1
            else:
                card_numbers[str(card_number)] += 1
            first_string, second_string = only_numbers.split('|') 
            first_numbers = [int(number) for number in first_string.strip().split()]
            second_numbers = [int(number) for number in second_string.strip().split()]
            intersection = set(first_numbers).intersection(second_numbers)
            card_copies = len(intersection)
            original_card_number = card_number
            for i in range(len(intersection)):
                card_number += 1
                if not card_numbers.get(str(card_number)):
                    card_numbers[str(card_number)] = 1 * card_numbers[str(original_card_number)]
                else:
                    card_numbers[str(card_number)] += 1 * card_numbers[str(original_card_number)]
